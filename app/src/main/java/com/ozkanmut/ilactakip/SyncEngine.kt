...
object SyncEngine {
    ...
    fun pullOnce(c: Context) = thread { DosefolkSyncScheduler.ensure(c.applicationContext); pullBlocking(c.applicationContext) }
        ...
        return try {
            ...
            if(connection.responseCode !in 200..299){connection.errorStream?.close();connection.disconnect();false}else{
                ...
                connection.inputStream.bufferedReader().useLines{lines->lines.forEach{line->
                    ...
                    // Stock uses a deliberately small owner-authored snapshot protocol rather than
                    // pretending to be a dose event. Consume it before DoseEvent parsing.
                    ...
                    StockEngine.applyEvent(context,event);applyRemoteState(context,event)
                }}
