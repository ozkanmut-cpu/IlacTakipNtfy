...
object StockEngine {
    ...
    @Synchronized fun clearRemoteOwnerChecked(c:Context,ownerId:String):Boolean{
        if(!saveRemoteChecked(c,loadRemote(c).filterNot{it.first==ownerId}))return false
        val p=prefs(c); val edit=p.edit(); p.all.keys.filter{it.startsWith("remote_rev|$ownerId|")||it.startsWith("remote_actor|$ownerId|")||it.startsWith("remote_event|$ownerId|")}.forEach(edit::remove); return edit.commit()
    }
    ...
    @Synchronized fun applyEvent(c:Context,event:DoseEvent){
        applyEventChecked(c,event)
    }
    @Synchronized fun applyEventChecked(c:Context,event:DoseEvent):Boolean{
        val stockTypes=setOf("taken","prn_taken","undo_taken","conflict_resolved_taken","conflict_resolved_missed")
        if(event.type !in stockTypes||alreadyProcessed(c,event.eventId))return true
        val ownerId=event.ownerId.ifBlank{event.actorTopic}; if(ownerId.isNotBlank()&&ownerId!=OwnerScopeStore.localOwnerId(c))return true
    ...
    private fun saveRemote(c:Context,v:List<Pair<String,MedicationStock>>){saveRemoteChecked(c,v)}
    private fun saveRemoteChecked(c:Context,v:List<Pair<String,MedicationStock>>):Boolean=prefs(c).edit().putString(KEY_REMOTE,remoteJson(v).toString()).commit()
}
