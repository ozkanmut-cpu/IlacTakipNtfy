...
object AssistantLocalRouter {
    fun execute(c: Context, raw: String): String {
        ...
        val todayWords = listOf(
            "bugün", "today", "heute", "aujourd", "hoy", "oggi", "hoje", "сегодня", "сьогодні",
            "اليوم", "امروز", "היום", "σήμερα", "astăzi", "idag", "i dag", "tänään", "dnes", "ma",
            "hari ini", "hôm nay", "आज", "今日", "오늘", "今天"
        )
        if (todayWords.any { q.contains(it) }) {
            return if (groups.isEmpty()) I18n.t("all_good")
            else groups.entries.joinToString("\n") { (time, list) -> "$time: ${list.joinToString { it.name }}" }
        }

        val nextWords = listOf(
            "sıradaki", "sonraki", "next", "nächste", "prochain", "siguiente", "prossim", "próxim",
            "volgende", "następ", "следующ", "наступн", "التالي", "بعدی", "הבא", "επόμεν", "următor",
            "nästa", "næste", "neste", "seuraava", "další", "következő", "berikut", "tiếp theo", "अगली",
            "次", "다음", "下一个"
        )
        if (nextWords.any { q.contains(it) }) {
            val now = LocalTime.now()
            val next = groups.entries.firstOrNull {
                runCatching { LocalTime.parse(it.key) }.getOrNull()?.isAfter(now) == true
            } ?: groups.entries.firstOrNull()
            return next?.let { "${it.key}: ${it.value.joinToString { med -> med.name }}" }
                ?: I18n.t("all_good")
        }

        val timeRegex = Regex("(?:[01]?\\d|2[0-3])[:.]?[0-5]\\d")
        val mentioned = timeRegex.find(q)?.value?.replace('.', ':')?.let { value ->
            if (value.contains(':')) value.padStart(5, '0')
            else value.padStart(4, '0').let { "${it.substring(0, 2)}:${it.substring(2)}" }
        }
        val target = if (mentioned != null) groups[mentioned]?.let { mentioned to it }
        else groups.entries.firstOrNull()?.let { it.key to it.value }

        val snoozeWords = listOf(
            "ertele", "snooze", "delay", "verschieb", "report", "pospon", "posticip", "adiar", "отлож",
            "відклад", "تأجيل", "تعویق", "דחה", "αναβολ", "amân", "skjut", "udsæt", "utsett", "siirrä",
            "odlož", "halaszt", "tunda", "hoãn", "स्थगित", "延期", "미루"
        )
        if (snoozeWords.any { q.contains(it) }) {
            if (target == null) return I18n.t("all_good")
            val minutes = Regex("(\\d+)\\s*(dk|dakika|min|minute|minuto|minuten)")
                .find(q)?.groupValues?.getOrNull(1)?.toIntOrNull() ?: 30
        ...
        val takenWords = listOf(
            "içtim", "içildi", "aldım", "taken", "took", "genommen", "pris", "tomado", "assunto",
            "ingenomen", "przyję", "принял", "прийня", "تم التناول", "مصرف", "נלקח", "ελήφθη", "luat",
            "tagen", "taget", "tatt", "otettu", "užito", "bevéve", "diminum", "đã uống", "ले लिया",
            "服用", "복용", "已服"
        )
        ...
        val missedWords = listOf(
            "içilmedi", "almadım", "kaçırdım", "not taken", "missed", "nicht genommen", "non pris",
            "no tomado", "não tomado", "niet ingenomen", "nie przyję", "не принял", "не прийня", "لم يتم",
            "مصرف نشد", "לא נלקח", "δεν ελήφθη", "neluat", "inte tagen", "ikke taget", "ikke tatt",
            "ei otettu", "neužito", "nem vette", "belum diminum", "chưa uống", "नहीं लिया", "未服用", "복용 안"
        )
        ...
        return "Dosefolk AI is ready for this command; secure action routing will handle advanced requests."
    }
}
