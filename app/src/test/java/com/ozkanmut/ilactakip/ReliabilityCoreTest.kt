...
class ReliabilityCoreTest {
    ...
    @Test fun eventStore_deduplicatesSameEventId() {
    ...
    @Test fun stock_takenIsIdempotentAndUndoRestoresDose() {
    ...
    @Test fun remoteStock_olderSnapshotCannotRollBackNewerState() {
    ...
    @Test fun prescriptionLifecycle_dueDoesNotSurfaceSupersededOldCycle() {
        ...
        val newer = PrescriptionRecord(id = "new", medicationName = "VASOXEN 5 MG 28 FILM TABLET", fillDate = "01.08.2026", doseEndDate = "01.12.2026", continuous = true)
        ...
        assertTrue(due.none { MedicationIdentity.canonical(it.medicationName) == "vasoxen" })
    }

    @Test fun medicationIdentity_matchesSgkPackagingNoiseToLocalName() {
        assertTrue(MedicationIdentity.same("Vasoxen", "VASOXEN 5 MG 28 FILM TABLET"))
        assertTrue(MedicationIdentity.same("Euthyrox", "EUTHYROX 50 MCG 100 TABLET"))
    }

    @Test fun sgkStockImport_addsNewCycleOnlyOnce() {
        ...
        MedicationMetaStore.save(c, MedicationMeta(
            medicationId = sgkMed.id, form = MedicationForm.TABLET, quantity = 1.0,
            packageCount = 28, packageUnit = "tablet", source = "sgk_pdf", doseUnitOverride = "tablet"
        ))
        PrescriptionRecordStore.upsertAll(c, listOf(PrescriptionRecord(
            id = "sgk-cycle-1", medicationId = sgkMed.id, medicationName = sgkMed.name,
            prescriptionNo = "RX1", prescriptionDate = "21.08.2026", fillDate = "21.08.2026",
            doseEndDate = "16.10.2026", boxCount = 2, dosePattern = "1x1", continuous = true
        )))
    ...
    @Test fun sgkStockImport_reusesExistingMedicationDespiteStrengthAndPackText() {
        val local = Medication("local-vasoxen", "Vasoxen", "1 tablet", listOf("08:00"))
        Store.save(c, listOf(local))
        MedicationMetaStore.save(c, MedicationMeta(
            medicationId = local.id, form = MedicationForm.TABLET, quantity = 1.0,
            packageCount = 28, packageUnit = "tablet", source = "manual", doseUnitOverride = "tablet"
        ))
        PrescriptionRecordStore.upsertAll(c, listOf(PrescriptionRecord(
            id = "variant-cycle", medicationName = "VASOXEN 5 MG 28 FILM TABLET",
            prescriptionNo = "RX2", prescriptionDate = "01.09.2026", fillDate = "01.09.2026",
            doseEndDate = "28.10.2026", boxCount = 2, dosePattern = "1x1", continuous = true
        )))

        SgkStockAutoImporter.reconcile(c)

        assertEquals(1, Store.load(c).count { MedicationIdentity.canonical(it.name) == "vasoxen" })
        assertEquals(56, StockEngine.forMedication(c, local.id)?.remainingDoses)
        assertEquals(local.id, PrescriptionRecordStore.all(c).first { it.prescriptionNo == "RX2" }.medicationId)
    }
}
