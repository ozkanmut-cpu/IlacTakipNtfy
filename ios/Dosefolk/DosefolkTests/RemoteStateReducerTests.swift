...
final class RemoteStateReducerTests: XCTestCase {
    ...
    func testProgramRevisionRejectsOlderRemoteUpdate() throws {
        let newer = event(type: "program_updated", eventId: "newer", revision: 5,
                          medication: Medication(id: "m1", name: "New", dose: "1", times: ["09:00"]))
        let older = event(type: "program_updated", eventId: "older", revision: 4,
    ...
    func testTargetedPresenceAndCapabilitiesArePersisted() throws {
        var presenceEvent = event(type: "circle_presence", eventId: "presence")
        presenceEvent.targetTopic = "local-topic"
        try reducer.apply(presenceEvent)
    ...
    func testUntargetedPresenceDoesNotConfirmPairing() throws {
        try reducer.apply(event(type: "circle_presence", eventId: "presence"))
        let presence = try store.load([String: CirclePresenceEntry].self, from: .circlePresence, default: [:])
        XCTAssertNil(presence["publisher-topic"])
    }
    ...
    func testRemoteRevokeFencesPeerAndClearsAllScopedPeerState() throws {
        try store.save([CirclePeer(id: "peer", name: "Peer", topic: "publisher-topic")], to: .circlePeers)
        try store.save([ScopedMedication(ownerId: "publisher-topic", medication: Medication(id: "m1", name: "Drug", dose: "1", times: ["08:00"]), medicationMeta: [])], to: .remoteMedications)
        try store.save([ScopedProgramRule(ownerId: "publisher-topic", rule: ProgramRule(medicationId: "m1"))], to: .remoteProgramRules)
        try store.save([ScopedStockState(ownerId: "publisher-topic", stock: StockState(medicationId: "m1", medicationName: "Drug", remainingDoses: 3, packSize: 10, updatedAt: 10))], to: .remoteStock)
        try store.save(["publisher-topic|m1": StockOrderingEntry(revision: 2, actorTopic: "publisher-topic", eventId: "stock", updatedAt: 10)], to: .stockOrdering)
        var presenceEvent = event(type: "circle_presence", eventId: "presence")
        presenceEvent.targetTopic = "local-topic"
        try reducer.apply(presenceEvent)
        try reducer.apply(event(type: "capability_edit_program_granted", eventId: "cap"))
        ...
        XCTAssertTrue(try store.load([ScopedMedication].self, from: .remoteMedications, default: []).isEmpty)
        XCTAssertTrue(try store.load([ScopedProgramRule].self, from: .remoteProgramRules, default: []).isEmpty)
        XCTAssertTrue(try store.load([ScopedStockState].self, from: .remoteStock, default: []).isEmpty)
        XCTAssertTrue(try store.load([String: StockOrderingEntry].self, from: .stockOrdering, default: [:]).isEmpty)
    }

    private func event(type: String, eventId: String, snoozeUntil: Int64 = 0, revision: Int64 = 0,
                       medication: Medication? = nil) -> DoseEvent {
        DoseEvent(eventId: eventId, type: type, time: "08:00", scheduledDate: "2026-09-11",
                  snoozeUntil: snoozeUntil, ownerId: "local-topic", actorTopic: "publisher-topic",
                  timestamp: 1_789_000_000_000, revision: revision,
